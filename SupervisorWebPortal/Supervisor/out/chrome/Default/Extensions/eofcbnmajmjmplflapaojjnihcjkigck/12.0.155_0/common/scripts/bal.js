  ...
  AvastWRC.bal = {
    ...
    init: function(browserType, _back, locStorage, editionConfig, forceEdition) {
      ...
      if (_.isArray(editionConfig)) {
    	  safePrice = editionConfig[0].safePrice;
    	  brandingType = editionConfig[0].brandingType;
      }
      else {
    	  safePrice = editionConfig.safePrice;
    	  brandingType = editionConfig.brandingType;
      }
        
      /* AOSP-639, AOSP-694*/
      if (safePrice && !AvastWRC.getStorage("landingPageShown")) {
          this.openLandingPageTab(brandingType);
      }      
    ...
    AvastWRC.bal.sp = _.extend( AvastWRC.bal.SP || {}, {
      ...
      onUrlInfoResponse : function (url, response, tab, tabUpdate) {
        ...
        if (safeShopData && tabUpdate)
        {
          ...
          chrome.browserAction.setIcon({ path: "common/skin/img/logo-safeprice-128.png", tabId: tabId });
        }
      },
        ...
        var queryOptions = {
          ...
          callback : function(offersResponse) {
            ...
            var showData = {
              domain : urlDomain,
              showHelp: showHelp,
              discreetSlider : !(AvastWRC.Utils.getDomain(tab.url) ==
                  AvastWRC.Utils.getDomain(AvastWRC.TabReqCache.get(tab.id, 'referer'))) &&
                  !showHelp,
