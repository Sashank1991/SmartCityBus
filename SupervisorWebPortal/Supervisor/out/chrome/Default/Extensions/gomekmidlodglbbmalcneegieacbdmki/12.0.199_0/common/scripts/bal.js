     ...
     */
        tabPhishing: function(url, rating, tab) {
            ...
            if(features.phishing) { //phishing and malware
                if(AvastWRC.bal.settings.get().phishing.trusted[url] !== true && (rating.getPhishing() > 1 )|| (rating.getRatingLevel() == 0 && rating.getPhishingDomain() > 1)) {
                    data = {
                        message : "showMessageBox",
                        data: {
                            type: (rating.getPhishing() > 1 )?"phishing":"bad_rating",
